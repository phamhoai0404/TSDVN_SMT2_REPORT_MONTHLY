...
{
    public class WriteOKI
    {
        public static void GetWriteOKI(ExcelWorksheet ws, List<DataDD> listDD, List<DataError> listError, string date, string typeKH)
        {
            ...
            if (numberErr1 > 0)
            {
                ...
                object[,] data333 = new object[1, 13];
                data333[0, 0] = value.Qty_Nham_LK == 0 ? (long?)null : value.Qty_Nham_LK;
                data333[0, 1] = value.Qty_Lech == 0 ? (long?)null : value.Qty_Lech;
                data333[0, 2] = value.Qty_Thieu_LK == 0 ? (long?)null : value.Qty_Thieu_LK;
                data333[0, 3] = value.Qty_Vo == 0 ? (long?)null : value.Qty_Vo;
                data333[0, 4] = value.Qty_Baccau == 0 ? (long?)null : value.Qty_Baccau;
                data333[0, 5] = value.Qty_Kenh == 0 ? (long?)null : value.Qty_Kenh;
                ...
                data333[0, 8] = value.Qty_Di_Vat == 0 ? (long?)null : value.Qty_Di_Vat;
                data333[0, 9] = value.Qty_Thua_LK == 0 ? (long?)null : value.Qty_Thua_LK;
                data333[0, 10] = value.Qty_Bong == 0 ? (long?)null : value.Qty_Bong;
                data333[0, 11] = value.Qty_Dung_dung == 0 ? (long?)null : value.Qty_Dung_dung;
                data333[0, 12] = (value.Qty_Khac + value.Qty_It_Thiec + value.Qty_SaiVitri +value.Qty_HanGia) == 0 ? (long?)null : (value.Qty_Khac + value.Qty_It_Thiec + value.Qty_SaiVitri + value.Qty_HanGia);
                ws.Cells[$"F{rowFirst}:R{rowFirst}"].Value = data333;

                //DAY LA VIET CHI TIET
                int currentRow = 7;
                var listErr = GetErrorOKI(listError, typeKH);
                ...
                var listErr222 = GetErrorOKI2(listError, typeKH);
        ...
        private static List<DataError3> GetErrorOKI(List<DataError> listTemp, string khachhang)
        {
            return listTemp.Where(p => khachhang.Contains(p.KH) && 
            (   p.QtyType.Qty_Khac != 0 ||
                p.QtyType.Qty_It_Thiec != 0 ||
                p.QtyType.Qty_SaiVitri != 0 ||
                p.QtyType.Qty_HanGia != 0
            )
        ...
        private static List<DataError4> GetErrorOKI2(List<DataError> listTemp, string khachhang)
        {
